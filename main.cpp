...
int main ()
{
  ...
  **/
  PID steer1 = PID();
  steer1.Init(0.5, 0.0005, 0.05, 1.2 ,-1.2);
  ...
  **/
  ...
  PID throttle1 = PID();
  throttle1.Init(0.2, 0.0005, 0.0005, 1, -1);

  h.onMessage([&steer1, &throttle1, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        auto a = hasData(data);

        if (a!= "") {

          auto data = json::parse(a);

          // opening 2 files for steer and throttle and save data
          fstream file_steer;
...
//           // Update the delta time with the previous command
           steer1.UpdateDeltaTime(new_delta_time);
...
//           error_steer = 0;
         double min_distance{std::numeric_limits<double>::infinity()};
          int min_dist_idx{0};
          for (int i=0; i < x_points.size(); ++i) {
            double distance = hypot(x_points[i] -x_position, y_points[i] - y_position);
          ...
          error_steer = correction(error_steer);

          /**
          * TODO (step 3): uncomment these lines
          **/
//           // Compute control to apply
           steer1.UpdateError(error_steer);
           steer_output = steer1.TotalError();
...
//           // Save data
          ...
          **/
//           // Update the delta time with the previous command
          throttle1.UpdateDeltaTime(new_delta_time);
          ...
          **/
//           // Compute control to apply
          throttle1.UpdateError(error_throttle);
          double throttle = throttle1.TotalError();
          ...
          msgJson["update_point_thresh"] = 40;
